...
func TestOpenStoreSeedsDefaults(t *testing.T) {
	...
	if settings.IdleThresholdSeconds != 60 || settings.MaxCountableGapSeconds != 300 ||
		settings.ChartYMaxMinutes != 720 || settings.BoardRefreshSeconds != 60 ||
...
func TestSetDeviceLockPersistsInConfig(t *testing.T) {
	store := testStore(t)
	ctx := context.Background()
	device, err := store.EnsureDevice(ctx, "client-1", fixedNow())
	if err != nil {
...
func TestSetDeviceLockUnlockClearsMessage(t *testing.T) {
	store := testStore(t)
	ctx := context.Background()
	device, err := store.EnsureDevice(ctx, "client-1", fixedNow())
	if err != nil {
...
func TestDevicesReportLockState(t *testing.T) {
	store := testStore(t)
	ctx := context.Background()
	device, err := store.EnsureDevice(ctx, "client-1", fixedNow())
	if err != nil {
	...
	if len(devices) != 1 || !devices[0].Locked {
		t.Fatalf("devices = %+v, want one locked device", devices)
	}
}
...
func TestUpdateSettingsPersistsAndRejectsNonPositive(t *testing.T) {
	...
	badBoard := settings
	badBoard.BoardRefreshSeconds = 0
	if err := store.UpdateSettings(ctx, badBoard, fixedNow()); err == nil {
		t.Fatalf("expected error for non-positive board_refresh_seconds")
	}
}
