...
.DESCRIPTION
  Copies the binaries to Program Files, writes the client config under
  ...
  service. Run from an elevated PowerShell prompt.
.PARAMETER ServerUrl
...
.PARAMETER SourceDir
  Directory containing rootaika-service.exe and rootaika-agent.exe.
  Defaults to ..\dist relative to this script (see build.ps1).
.EXAMPLE
...
$serviceExe  = Join-Path $installDir "rootaika-service.exe"
$agentExe    = Join-Path $installDir "rootaika-agent.exe"
$runKey      = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
$runValue    = "rootaika-agent"

$srcService = Join-Path $SourceDir "rootaika-service.exe"
$srcAgent   = Join-Path $SourceDir "rootaika-agent.exe"
if (-not (Test-Path $srcService) -or -not (Test-Path $srcAgent)) {
    throw "Binaries not found in $SourceDir. Run build.ps1 first."
}

Write-Host "Installing binaries to $installDir..."
New-Item -ItemType Directory -Force -Path $installDir | Out-Null
New-Item -ItemType Directory -Force -Path $dataDir | Out-Null
Copy-Item $srcService $serviceExe -Force
Copy-Item $srcAgent $agentExe -Force
...
$binPath = "`"$serviceExe`" -config `"$configPath`""
...
Set-ItemProperty -Path $runKey -Name $runValue -Value "`"$agentExe`" -config `"$configPath`""
